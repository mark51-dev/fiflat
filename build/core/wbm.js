    ...
    async parseAllLatestDetails(listOfItems) {
        for (let i = 0; i < listOfItems.length; i++) {
            ...
            const res = await got
                ...
                .text();
            const $ = load(res);
            const price = $('#description > div.textWrap > ul:nth-child(5) > li:nth-child(3) > span.value.big-1')
                .text()
                .replace('EUR', '')
                .trim();
            const rooms = $('#description > div.textWrap > ul:nth-child(9) > li:nth-child(1)')
                .text()
                .replaceAll('\t', '')
                .replaceAll('\n', '')
                .replace('Anzahl der Zimmer: ', '');
            const area = $('#description > div.textWrap > ul:nth-child(9) > li:nth-child(2)')
                .text()
                .replaceAll('\t', '')
                .replaceAll('\n', '')
                .replace('Größe: ca. ', '');
